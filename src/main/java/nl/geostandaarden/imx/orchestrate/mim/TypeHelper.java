...
public class TypeHelper {
    ...
    public static String getValueTypeName(Keuze keuze) {
        if (keuze.getDatatypen().stream().allMatch(dt -> dt.getNaam().startsWith("GM_"))) {
            return "Geometry";
        }

        if (keuze.getDatatypen().stream()
                        .map(TypeHelper::resolveName)
                        .distinct()
                        .count()
                <= 1) {
            return keuze.getDatatypen().stream()
                    .findFirst()
    ...
    public static String getValueTypeName(@NonNull Datatype datatype) {
        ...
        if (datatype.getSupertypes(true).stream()
                .map(Modelelement::getNaam)
                .anyMatch(isEqual(MimDatatypes.CHARACTER_STRING))) {
            return STRING.getName();
        }

        if (datatype.getNaam().startsWith("GM_")) {
            return "Geometry";
        }
        ...
        return switch (resolvedName) {
            case MimDatatypes.CHARACTER_STRING,
                    MimDatatypes.DATE,
                    MimDatatypes.DATETIME,
                    MimDatatypes.YEAR,
                    MimDatatypes.URI -> STRING.getName();
