...
public class MimModelMapper {
    ...
    public Model fromModel(Informatiemodel informatiemodel) {
        ...
        informatiemodel.getPackages().forEach(mimPackage -> processPackage(mimPackage, modelBuilder));

        return modelBuilder.build();
    }

    private void processPackage(Package mimPackage, Model.ModelBuilder modelBuilder) {
        mimPackage.getObjecttypen().stream()
                .filter(not(Objecttype::isIndicatieAbstractObject))
                .map(this::toObjectType)
                .forEach(modelBuilder::objectType);

        mimPackage.getGegevensgroeptypen().stream().map(this::toObjectType).forEach(modelBuilder::objectType);

        mimPackage.getDatatypen().stream()
    ...
    private Set<Property> processProperties(Objecttype objecttype) {
        return Stream.of(
                        objecttype.getAttribuutsoorten(true).stream().map(this::toProperty),
                        objecttype.getRelatiesoorten(true).stream()
                                .filter(relatiesoort -> relatiesoort.getDoel() != null)
                                .map(this::toRelation),
                        objecttype.getGegevensgroepen(true).stream().map(this::toRelation))
    ...
    private Set<Property> processProperties(Gegevensgroeptype gegevensgroeptype) {
        return Stream.of(
                        gegevensgroeptype.getAttribuutsoorten().stream().map(this::toProperty),
                        gegevensgroeptype.getRelatiesoorten().stream().map(this::toRelation),
                        gegevensgroeptype.getGegevensgroepen().stream().map(this::toRelation))
                .reduce(Stream::concat)
                .orElseGet(Stream::empty)
                .collect(toUnmodifiableSet());
    }
    ...
    private Set<Property> processProperties(GestructureerdDatatype gestructureerdDatatype) {
        return gestructureerdDatatype.getDataElementen().stream()
                .map(this::toProperty)
                .collect(toUnmodifiableSet());
    }
    ...
    private Property toProperty(Attribuutsoort attribuutsoort) {
        if (isScalarLike(attribuutsoort)) {
            ...
            var valueType = valueTypeRegistry
                    .getValueTypeFactory(valueTypeName)
    ...
    private Relation toRelation(Relatiesoort relatiesoort) {
        return Relation.builder()
                ...
                .inverseMultiplicity(
                        getMultiplicityOrDefault(relatiesoort.getInverseKardinaliteit(), Multiplicity.MULTI))
                .build();
    }
